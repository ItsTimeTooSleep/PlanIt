...
import { CHART_COLORS, DEFAULT_TAG_COLOR } from "@/lib/colors";
...
export function StatsView() {
	...
	const interval = useMemo(
		() => getInterval(range, customFrom, customTo),
		[range, customFrom, customTo],
	);
	const prevInterval = useMemo(
		() => getPrevInterval(range, interval),
		[range, interval],
	);
	...
	const curTasks = useMemo(
		...
		[interval, filterTasks],
	);
	const prevTasks = useMemo(
		() => filterTasks(prevInterval),
		[prevInterval, filterTasks],
	);
	...
	const focusMinsCur = completedCur.reduce(
		(s, t) => s + taskDurationMinutes(t),
		0,
	);
	const focusMinsPrev = completedPrev.reduce(
		(s, t) => s + taskDurationMinutes(t),
		0,
	);
	...
	const pieData = Object.entries(tagMinutes).map(([tagId, minutes]) => {
	...
	const dailyCur = days.map((d) => {
		...
		return {
			date: lang === "zh" ? format(d, "M/d") : format(d, "M/d"),
						...
						{pieData.length > 0 && (
							<div className="bg-card border border-border rounded-xl p-4">
								<h2 className="text-sm font-semibold mb-1">
									{t.stats.tagBreakdown}
								</h2>
								<p className="text-xs text-muted-foreground mb-3">
									{t.stats.tagBreakdownDesc}
								</p>
