...
function HomeViewContent() {
	...
	function openCreate(startMin?: number) {
		...
		const end =
			startMin !== undefined
				? minutesToTime(Math.min(startMin + 60, 23 * 60))
				: undefined;
		...
		<div className="flex flex-col h-[calc(100vh-2.25rem)] bg-background">
			...
			>
				<div
					...
					style={{ gap: isFabHovered ? "8px" : "0px" }}
				>
					<Plus
						className={`w-6 h-6 flex-shrink-0 transition-transform duration-300 ${isFabHovered ? "rotate-90" : ""}`}
					/>
					<span
