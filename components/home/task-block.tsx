...
function isTaskCurrentlyExecuting(task: Task): boolean {
	if (!task.startTime || !task.endTime || task.status !== "pending")
		return false;
	const now = new Date();
...
export function TaskBlock({
	...
	const _top =
		overrideTop !== undefined ? overrideTop : (startMin / 60) * hourHeight;
	const height =
	...
	const fontSize =
		height < 24 ? "text-[9px]" : height < 36 ? "text-[10px]" : "text-xs";
					...
					<div className="flex items-start gap-1.5 px-2 py-1 h-full">
						<button
							...
							style={{
								...
								backgroundColor: isDone ? primaryColor : "transparent",
							}}
						>
							{isDone && (
								<svg
							...
							<div className="flex items-center gap-1.5 min-w-0">
								...
								{isActivePomodoroTask && (
									<span
										...
										style={{
											backgroundColor: POMODORO_COLORS.work,
