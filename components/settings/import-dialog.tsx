...
export function ImportDialog({
	...
	const actualData: AppState =
		"meta" in parsedData ? parsedData.data : parsedData;
	...
	const importItems: {
		...
		{
			key: "dateNotes",
			label: t.settings.importDateNotes,
			count: counts.dateNotes,
		},
		{ key: "notes", label: t.settings.importNotes, count: counts.notes },
		{
			key: "noteLines",
			label: t.settings.importNoteLines,
			count: counts.noteLines,
		},
					...
					{hasMeta && (
						<div className="text-xs text-muted-foreground bg-muted/30 rounded-md px-3 py-2">
							...
							<div className="flex justify-between mt-1">
								...
								<span>
									{new Date(
										(parsedData as ExportData).meta.exportDate,
									).toLocaleString()}
								</span>
							</div>
						</div>
						...
						<div className="flex gap-3">
							...
							>
								<div className="font-medium">
									{t.settings.importOverwriteTitle}
								</div>
								<div className="text-xs text-muted-foreground mt-1">
									{t.settings.importOverwriteDesc}
								</div>
							</button>
						</div>
					</div>
