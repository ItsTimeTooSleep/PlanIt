...
export function PomodoroSettings() {
	...
	function updatePomodoroSetting<K extends keyof typeof pomodoroSettings>(
		key: K,
		value: (typeof pomodoroSettings)[K],
	) {
		updatePomodoro({ settings: { ...pomodoroSettings, [key]: value } });
	}

	const formatOption = (value: number, unit: string) => ({
		value,
		label: `${value} ${unit}`,
	});
		...
		<div className="space-y-4">
			<div className="grid grid-cols-2 gap-x-6 gap-y-3">
				<SettingRow label={t.pomodoro.workDuration}>
					<SelectControl
						options={WORK_DURATION_OPTIONS.map((m) =>
							formatOption(m, t.settings.minutes),
						)}
				...
				<SettingRow label={t.pomodoro.shortBreakDuration}>
					<SelectControl
						options={SHORT_BREAK_OPTIONS.map((m) =>
							formatOption(m, t.settings.minutes),
						)}
			...
			<div className="grid grid-cols-2 gap-x-6 gap-y-3">
				<SettingRow label={t.pomodoro.longBreakDuration}>
					<SelectControl
						options={LONG_BREAK_OPTIONS.map((m) =>
							formatOption(m, t.settings.minutes),
						)}
				...
				<SettingRow label={t.pomodoro.workSessionsBeforeLongBreak}>
					<SelectControl
						options={SESSIONS_OPTIONS.map((n) =>
							formatOption(n, lang === "zh" ? "个" : "sessions"),
						)}
						value={pomodoroSettings.workSessionsBeforeLongBreak}
						onChange={(v) =>
							updatePomodoroSetting("workSessionsBeforeLongBreak", v)
						}
					/>
				</SettingRow>
			</div>
