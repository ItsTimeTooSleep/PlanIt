...
const CLOSE_BEHAVIORS: {
	value: CloseBehavior;
	labelKey: "closeBehaviorExit" | "closeBehaviorTray";
}[] = [
...
export function GeneralSettings() {
	...
	}, [shouldShowDesktopSettings, api, state.settings.closeBehavior]);
	...
	return (
		<div className="space-y-4">
			...
			<SettingRow
				label={t.settings.startupPage}
				description={t.settings.startupPageDesc}
			>
				<Select
					value={state.settings.startupPage}
					onValueChange={(value) =>
						updateSettings({ startupPage: value as StartupPage })
					}
				>
			...
			{shouldShowDesktopSettings && (
				<SettingGroup bordered>
					...
					>
						<SegmentedControl
							options={CLOSE_BEHAVIORS.map((b) => ({
								value: b.value,
								label: t.settings[b.labelKey],
							}))}
