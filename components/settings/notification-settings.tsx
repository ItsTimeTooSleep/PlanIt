...
export function NotificationSettings() {
	...
	useEffect(() => {
		if (isDesktop()) {
			import("@tauri-apps/plugin-notification")
				...
				.catch((error) => {
					console.error(
						`${LOG_PREFIX} Failed to check desktop permission:`,
						error,
					);
	...
	async function handleToggleNotifications(enabled: boolean) {
		console.log(
			`${LOG_PREFIX} handleToggleNotifications called with enabled:`,
			enabled,
		);
		...
		if (isDesktop()) {
			...
			if (enabled) {
				try {
					...
					if (!granted) {
						console.log(
							`${LOG_PREFIX} Requesting desktop notification permission...`,
						);
					...
					if (granted) {
						console.log(
							`${LOG_PREFIX} Permission granted, enabling notifications`,
						);
						updateSettings({
							notifications: {
					...
					console.error(
						`${LOG_PREFIX} Error handling desktop notification permission:`,
						error,
					);
				}
			} else {
			...
			console.log(
				`${LOG_PREFIX} Browser environment, current permission:`,
				Notification.permission,
			);

			if (Notification.permission === "granted") {
				console.log(
					`${LOG_PREFIX} Permission already granted, enabling notifications`,
				);
				updateSettings({
					notifications: {
						...state.settings.notifications,
				...
				console.warn(
					`${LOG_PREFIX} Permission denied by user, cannot enable notifications`,
				);
			...
			console.log(
				`${LOG_PREFIX} Notifications not supported or disabling, updating settings`,
			);
			updateSettings({
				notifications: {
					...state.settings.notifications,
	...
	console.log(
		`${LOG_PREFIX} Current state - notificationsEnabled:`,
	...
	return (
		<div className="space-y-4">
			<SettingGroup bordered>
				<SettingRow
					...
					description={
						isDesktopEnv
							...
							: t.settings.notificationsDescWeb
					}
				>
				...
				>
					<SettingRow
						label={t.settings.advanceNotification}
						description={t.settings.advanceNotificationDesc}
					>
						<SelectControl
					...
					<SettingRow
						label={t.settings.showStartNotification}
						description={t.settings.showStartNotificationDesc}
					>
						<Switch
							checked={
								state.settings.notifications?.showStartNotification ?? true
							}
					...
					<SettingRow
						label={t.settings.showEndNotification}
						description={t.settings.showEndNotificationDesc}
					>
						<Switch
							checked={
								state.settings.notifications?.showEndNotification ?? false
							}
