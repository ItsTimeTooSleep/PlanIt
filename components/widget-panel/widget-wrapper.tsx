...
import { Lock, LockOpen, Move, X } from "lucide-react";
import { memo, useCallback, useEffect, useRef, useState } from "react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { clampSize, snapToGrid } from "@/lib/widget-layout-manager";
import { WIDGET_METADATA } from "@/lib/widget-registry";
import type { Position, Size, WidgetInstance } from "@/lib/widget-types";
...
export const WidgetWrapper = memo(function WidgetWrapper({
	...
	const safeCanvasSize = canvasSize || { width: Infinity, height: Infinity };
	...
	useEffect(() => {
		...
		const handleMouseMove = (e: MouseEvent) => {
			if (isDragging && canvasRef?.current) {
				...
				newX = Math.min(newX, safeCanvasSize.width - pixelSize.width);
				newY = Math.min(newY, safeCanvasSize.height - pixelSize.height);
		...
		applyGridSnap,
		onEndBatchUpdate,
		safeCanvasSize.height,
		pixelSize.height,
		pixelSize.width,
		safeCanvasSize.width,
		safeCanvasSize,
	]);
	...
	return (
		...
		>
			...
			{!isLocked &&
				showResizeHandles &&
				(Object.keys(resizeHandleStyle) as ResizeDirection[]).map(
					(direction) => (
						<div
							key={direction}
							data-resize-handle
