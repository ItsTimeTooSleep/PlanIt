...
import { Grid3X3, LayoutGrid, RotateCcw, ZoomIn, ZoomOut } from "lucide-react";
import {
	lazy,
	Suspense,
	useCallback,
...
export function WidgetCanvas({
	...
	const handleZoomInputChange = useCallback((value: string) => {
		...
		if (!Number.isNaN(numValue) && numValue >= 25 && numValue <= 200) {
	...
	const handleDrop = useCallback(
		(e: React.DragEvent) => {
			...
			console.log(
				"[WidgetCanvas] calling addWidget with type:",
				widgetType,
				"position:",
				position,
			);
		...
		return (
			<div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-background/90 backdrop-blur-sm border border-border rounded-lg px-3 py-1.5 shadow-sm">
				...
				<Button
					...
					title={t.customLayout.restoreDefault}
				>
