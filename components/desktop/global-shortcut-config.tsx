...
export function GlobalShortcutConfig({ className }: GlobalShortcutConfigProps) {
				...
				await api.registerGlobalShortcut({
					...
					handler: () => {
						window.dispatchEvent(
							new CustomEvent(`shortcut:${shortcut.action}`),
						);
					},
				});
			} catch (error) {
				console.error(
					`[GlobalShortcutConfig] Failed to register ${shortcut.accelerator}:`,
					error,
				);
			}
		}
	...
	const handleSaveShortcut = useCallback(
		async (id: string, newAccelerator: string) => {
			if (!api) return;
			...
			try {
				...
				console.error(
					"[GlobalShortcutConfig] Failed to update shortcut:",
					error,
				);
			} finally {
				setEditingId(null);
