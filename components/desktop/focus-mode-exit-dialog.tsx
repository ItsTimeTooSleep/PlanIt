...
import { AlertTriangle, Heart, Shield, Sparkles } from "lucide-react";
...
const EXIT_PHRASES_ZH = ["不想努力了", "我要放弃", "让我出去", "坚持不住了"];
 ...
 */
export function FocusModeExitDialog({
	isOpen,
	onClose,
	onConfirm,
}: FocusModeExitDialogProps) {
	...
	}, [inputValue, targetPhrase, randomizeEncouragement]);
	...
	}, [holdProgress, randomizeEncouragement]);
					...
					{step === "initial" && (
						<div className="space-y-4">
							...
							<div className="flex items-center gap-2 text-sm text-muted-foreground">
								<Heart
									className="w-4 h-4"
									style={{ color: "var(--tag-pink)" }}
								/>
								<span>
									{isZh
										? "我们相信你可以坚持到底！"
										: "We believe you can make it to the end!"}
								</span>
							</div>
						</div>
					)}
					...
					{step === "typing" && (
						<div className="space-y-4">
							...
							<Input
								...
								placeholder={
									isZh ? "输入上方文字..." : "Type the text above..."
								}
							...
							<div className="flex items-center gap-2 text-sm text-muted-foreground">
								...
								<span>
									{isZh
										? "再想一想，真的要放弃吗？"
										: "Think again, do you really want to give up?"}
								</span>
							</div>
						</div>
					)}
						...
						<div className="space-y-4">
							<div className="p-4 rounded-lg bg-muted/50">
								<p className="text-sm text-muted-foreground mb-2">
									{isZh
										? "按住下方按钮 5 秒以退出专注模式"
										: "Hold the button below for 5 seconds to exit focus mode"}
								</p>
							</div>
							...
							<div className="flex items-center gap-2 text-sm text-muted-foreground">
								...
								<span>
									{isZh
										? "这是最后一步，松手就会重置进度"
										: "This is the last step, releasing will reset progress"}
								</span>
							</div>
						</div>
					)}
