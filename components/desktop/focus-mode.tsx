...
export function FocusMode({
	...
	const getPhaseLabel = () => {
		switch (pomodoroState.phase) {
			case "work":
				return isZh ? "专注中" : "Focusing";
			case "shortBreak":
				return isZh ? "短休息" : "Short Break";
			case "longBreak":
				return isZh ? "长休息" : "Long Break";
		}
	};
	...
	return (
		<>
			<Dialog
				...
				onOpenChange={(open) => !open && handleEnterCancel()}
			>
				<DialogContent
					className="max-w-md"
					...
					<div className="py-4 space-y-4">
						...
						<div className="space-y-3">
							<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
								...
								<div>
									<p className="text-sm font-medium">
										{isZh ? "全屏专注" : "Fullscreen Focus"}
									</p>
									<p className="text-xs text-muted-foreground">
										{isZh
											? "番茄钟将占据整个屏幕"
											: "Pomodoro timer will take the entire screen"}
									</p>
								</div>
							</div>
							...
							<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
								...
								<div>
									<p className="text-sm font-medium">
										{isZh ? "屏蔽干扰" : "Block Distractions"}
									</p>
									<p className="text-xs text-muted-foreground">
										{isZh
											? "禁用 Alt+Tab、Win+D 等系统快捷键"
											: "Disable Alt+Tab, Win+D and other system shortcuts"}
									</p>
								</div>
							</div>
							...
							<div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{isZh
											? "专注于当前任务，直到番茄钟结束"
											: "Focus on your task until the pomodoro ends"}
									</p>
								</div>
							</div>
						</div>
						...
						>
							...
							<span>
								{isZh
									? "退出时需要多重确认，请确保有足够时间"
									: "Exit requires multiple confirmations, ensure you have enough time"}
							</span>
						</div>
					</div>
			...
			{!showEnterConfirm && (
				<div className="fixed inset-0 z-50 bg-background flex flex-col">
					...
					<div className="flex-1 flex flex-col items-center justify-center p-8">
						<div className="text-center mb-8">
							...
							<p className="text-sm text-muted-foreground">
								{isZh
									? `第 ${pomodoroState.workSessions + 1} 个番茄钟`
									: `Pomodoro #${pomodoroState.workSessions + 1}`}
							</p>
						</div>
					...
					<div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-muted-foreground">
						{isZh
							? "按 ESC 或点击右上角退出专注模式"
							: "Press ESC or click top right to exit focus mode"}
					</div>
				</div>
			)}
