...
function load(): AppState {
	try {
		...
		const migratedSettings: Partial<AppState["settings"]> = {
			...parsed.settings,
		};

		if (
			(migratedSettings as any).notificationsEnabled !== undefined &&
		...
		return {
			...
			settings: {
				...
				...migratedSettings,
				firstLaunchDate:
					migratedSettings.firstLaunchDate ??
...
function checkAndPlayTaskTimeSounds(tasks: Task[], settings: AppSettings) {
	...
	tasks.forEach((task) => {
		...
		const startMinutes =
			parseInt(task.startTime.split(":")[0], 10) * 60 +
			parseInt(task.startTime.split(":")[1], 10);
		const endMinutes =
			parseInt(task.endTime.split(":")[0], 10) * 60 +
			parseInt(task.endTime.split(":")[1], 10);

		// 检查任务开始
		if (
			Math.abs(nowMinutes - startMinutes) <= 1 &&
			settings.sound.playOnTaskStart
		) {
			playSound("taskStart");
		}
		...
		// 检查任务结束
		if (
			Math.abs(nowMinutes - endMinutes) <= 1 &&
			settings.sound.playOnTaskEnd
		) {
			playSound("taskEnd");
		}
	});
...
export function StoreProvider({ children }: { children: ReactNode }) {
	...
	const _scheduledRef = useRef(false);
	...
	}, [state.settings.notifications, state.settings.language, state.tasks]);
	...
	}, [hydrated, rescheduleAllNotifications]);
	...
	const addTask = useCallback(
		(task: Task, recordHistory: boolean = true) => {
			if (recordHistory) {
				historyManager.push({
					type: "create_task",
					timestamp: Date.now(),
	...
	const deleteTask = useCallback(
		(id: string, recordHistory: boolean = true) => {
	...
	const deleteTasks = useCallback(
		(ids: string[], recordHistory: boolean = true) => {
			...
			if (recordHistory) {
				set((prev) => {
					...
					const next = {
						...prev,
						tasks: prev.tasks.filter((t) => !s.has(t.id)),
					};
					save(next);
					return next;
				});
	...
	const getDateNote = useCallback(
		(date: string): DateNote | undefined => {
	...
	const deleteDateNote = useCallback(
		(date: string) => {
			set((prev) => ({
				...prev,
	...
	const updatePomodoro = useCallback(
		(
			...
			set((prev) => {
				const pomodoroUpdates =
					typeof updates === "function" ? updates(prev.pomodoro) : updates;
				return {
					...prev,
	...
	const updateNoteLine = useCallback(
		(id: string, updates: Partial<NoteLine>) => {
			set((prev) => ({
				...prev,
				noteLines: prev.noteLines.map((l) =>
					l.id === id ? { ...l, ...updates } : l,
				),
			}));
		},
	...
	const deleteNoteLine = useCallback(
		(id: string) => {
			set((prev) => ({
				...prev,
				noteLines: prev.noteLines.filter((l) => l.id !== id),
			}));
		},
		[set],
	);

	const getNoteLinesByDate = useCallback(
		(date: string): NoteLine[] => {
			...
			set((prev) => {
				...
				return {
					tasks: importOpts.tasks
						? [
								...prev.tasks,
								...actualData.tasks.filter((t) => !eIds.has(t.id)),
							]
						: prev.tasks,
					...
					dateNotes: importOpts.dateNotes
						? [
								...prev.dateNotes,
								...actualData.dateNotes.filter((n) => !eDateNotes.has(n.date)),
							]
						: prev.dateNotes,
					notes: importOpts.notes
						? [
								...prev.notes,
								...importedNotesWithZIndex.filter((n) => !eNotes.has(n.id)),
							]
						: prev.notes,
					noteLines: importOpts.noteLines
						? [
								...prev.noteLines,
								...actualData.noteLines.filter((l) => !eNoteLines.has(l.id)),
							]
						: prev.noteLines,
					settings: importOpts.settings ? actualData.settings : prev.settings,
					pomodoro: importOpts.pomodoro ? actualData.pomodoro : prev.pomodoro,
