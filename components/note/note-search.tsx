...
export function NoteSearchPanel({
			...
			if (titleMatch || contentMatch) {
				...
				if (titleMatch) {
					...
					const start = Math.max(
						0,
						contentText.toLowerCase().indexOf(query) - 20,
					);
	...
	const formatDateLabel = useCallback(
		...
		[lang, t.note.today, t.common.yesterday],
	);
	...
	const highlightMatch = useCallback((text: string, query: string) => {
		...
		return (
			<>
				{before}
				<mark className="bg-primary/30 text-foreground px-0.5 rounded">
					{match}
				</mark>
				{after}
			</>
							...
							<div className="flex flex-col items-center justify-center text-center space-y-4">
								...
								<div className="flex flex-wrap justify-center gap-2 mt-4">
									<div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/50 text-xs text-muted-foreground">
										...
										<span>{t.note.navigate}</span>
									</div>
									<div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/50 text-xs text-muted-foreground">
										<kbd className="px-1.5 py-0.5 rounded bg-background border text-[10px] font-mono">
											Enter
										</kbd>
										<span>{t.note.jump}</span>
									</div>
									<div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/50 text-xs text-muted-foreground">
										<kbd className="px-1.5 py-0.5 rounded bg-background border text-[10px] font-mono">
											Esc
						...
						<div className="px-5 py-12 text-center">
							...
							<p className="text-muted-foreground">
								{lang === "zh"
									...
									: `No notes found containing "${searchQuery}"`}
							</p>
							<p className="text-sm text-muted-foreground/70 mt-1">
								{t.note.tryDifferentKeywords}
							</p>
						</div>
					)}
						...
						<div className="flex items-center justify-between px-5 py-3 border-t border-border/50 bg-muted/30 text-xs text-muted-foreground">
							<div className="flex items-center gap-4">
								<span className="flex items-center gap-1">
									...
									<span className="ml-1">{t.common.select}</span>
								</span>
								<span className="flex items-center gap-1">
									<kbd className="px-1.5 py-0.5 rounded bg-background border text-[10px] font-mono">
										Enter
									</kbd>
									<span className="ml-1">{t.note.jumpToNote}</span>
								</span>
							</div>
							<span className="flex items-center gap-1">
								<kbd className="px-1.5 py-0.5 rounded bg-background border text-[10px] font-mono">
