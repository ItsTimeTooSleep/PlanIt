...
export function NoteEditor({
	...
	const execCommand = useCallback(
		(command: string, value?: string) => {
			...
			console.log(
				"[execCommand] 执行前 queryCommandValue(foreColor):",
				document.queryCommandValue("foreColor"),
			);
			document.execCommand(command, false, value);
			console.log(
				"[execCommand] 执行后 queryCommandValue(foreColor):",
				document.queryCommandValue("foreColor"),
			);
			handleInput();
		},
						...
						<ToggleGroup type="single" className="flex flex-col gap-1">
							{TEXT_SIZES.map((size) => (
								...
								>
									<span
										style={{
											fontSize: `${12 + parseInt(size.value, 10) * 2}px`,
										}}
									>
										{size.label}
									</span>
