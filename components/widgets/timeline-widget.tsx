...
import { sortTasksByTime } from "@/lib/task-utils";
...
export function TimelineWidget({
	...
	const hourHeight =
		(config?.hourHeight as number) ?? HOUR_HEIGHT_CONFIG[sizeMode];
	...
	const scrollToNow = useCallback(
		(behavior: ScrollBehavior = "smooth") => {
			...
			const totalDisplayMinutes =
				DEFAULT_FOCUS_MINUTES_BEFORE + DEFAULT_FOCUS_MINUTES_AFTER;
			const totalDisplayHeight = (totalDisplayMinutes / 60) * currentHourHeight;

			let targetTop: number;
			if (viewportHeight > totalDisplayHeight) {
				targetTop =
					(nowMinutes / 60) * currentHourHeight -
					viewportHeight / 2 +
					currentHourHeight * 0.5 +
					20;
			} else {
				const startMinutes = Math.max(
					0,
					nowMinutes - DEFAULT_FOCUS_MINUTES_BEFORE,
				);
	...
	useEffect(() => {
		// Initialize lastScrollRef on the client
	...
	useEffect(() => {
		...
		autoScrollTimerRef.current = setInterval(() => {
			if (
				...
				Date.now() - lastScrollRef.current >= 15_000
			) {
				scrollToNow("smooth");
			}
		}, 5_000);
	...
	return (
		<div
			ref={containerRef}
			className={cn(
				"relative flex-1 flex justify-center overflow-hidden",
				className,
			)}
		>
			<div className="w-full h-full relative rounded-xl bg-card border border-border shadow-sm overflow-hidden">
				<div
					className="pointer-events-none absolute top-0 left-0 right-0 h-20 z-20"
					style={{
						background:
							"linear-gradient(to bottom, var(--card) 0%, rgba(255,255,255,0.9) 40%, transparent 100%)",
					}}
				/>
				<div
					...
					style={{
						background:
							"linear-gradient(to top, var(--card) 0%, rgba(255,255,255,0.9) 40%, transparent 100%)",
					}}
				/>
					...
					>
						{showDayMarkers && (
							...
							>
								<div className="flex items-center gap-2 text-muted-foreground/60">
									...
									<span className={cn("font-medium", hourLabelFontSize)}>
										{t.timeline.newDay}
									</span>
									<div className="w-12 h-px bg-muted-foreground/40" />
								</div>
						...
						{showCurrentTimeLine && (
							...
							>
								<span
									className={cn(
										"absolute -translate-y-1/2 w-14 text-right pr-3 font-semibold text-primary tabular-nums select-none",
										hourLabelFontSize,
									)}
								>
									{format(now, "HH:mm")}
						...
						{timedTasks.length === 0 && (
							<div
								className="absolute pointer-events-none"
								style={{ top: nowTop + 52, left: TIMELINE_LEFT + 16 }}
							>
