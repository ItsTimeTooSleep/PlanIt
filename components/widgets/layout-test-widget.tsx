...
import { Clock, MessageSquare, Pause, Tag, Timer } from "lucide-react";
import {
	forwardRef,
	useCallback,
	useEffect,
...
import {
	DEFAULT_TASK_CONTENT_CONFIGS,
	useAdaptiveLayout,
} from "@/hooks/use-adaptive-layout";
 ...
 * @returns 测试组件
 */
export const LayoutTestWidget = forwardRef<
	LayoutTestWidgetRef,
	LayoutTestWidgetProps
>(function LayoutTestWidget({ className, contentOptions }, ref) {
	...
	const [effectiveContentOptions, setEffectiveContentOptions] = useState<
		ContentVisibilityOption[]
	>([]);
	...
	const mockTask = {
		...
		remaining: "1小时30分钟",
		notes:
			"这是一个任务备注，用于测试备注显示功能。当空间足够时会显示这段文字。",
	};
	...
	const adaptiveConfig = useMemo(() => {
		const contents = effectiveContentOptions
			...
			.map((opt) => {
				const config = DEFAULT_TASK_CONTENT_CONFIGS.find(
					(c) => c.id === opt.id,
				);
				return {
					id: opt.id,
	...
	useEffect(() => {
		...
		const calculateMetrics = (
			...
			const _buttonHeight = BUTTON_HEIGHT;
			...
			const effectiveContentWidth = needsButtonWrap
				? contentAreaWidth
				: contentWithInlineButton;
	...
	const displayTitle = useCallback(() => {
		...
		metrics.needsTitleTruncate,
		metrics.titleMaxChars,
		mockTask.title.slice,
		mockTask.title,
	]);
	...
	const getMinSize = useCallback(() => {
		...
		const progressHeight = effectiveContentOptions.find(
			(o) => o.id === "progress" && o.required,
		)
			? PROGRESS_HEIGHT
			: 0;
	...
	return (
		<div
			ref={containerRef}
			className={cn(
				"relative overflow-hidden flex flex-col h-full bg-card rounded-xl border border-border",
				className,
			)}
		>
			<div
				...
				style={{
					background: `linear-gradient(90deg, ${tagColor} 0%, transparent 50%)`,
				}}
			/>
			...
			>
				...
				>
					<Timer
						...
						style={{
							width: metrics.iconSize * 0.5,
							height: metrics.iconSize * 0.5,
							color: tagColor,
						}}
					/>
				</div>
