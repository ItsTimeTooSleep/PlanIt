...
export function SpinWheelWidget({
	...
	const showHistoryPanel =
		showHistory && history.length > 0 && sizeMode !== "compact";
				...
				<div className="p-3 border-b border-border bg-muted/20 space-y-2">
					<div className="flex gap-2">
						<Input
							...
							placeholder={`${t.widgets.addOption}...`}
			...
			<div className="flex-1 flex flex-col items-center justify-center p-4 relative">
				...
				>
					<svg viewBox="0 0 100 100" className="w-full h-full">
						{options.map((option, index) => {
							...
							const maxTextLength =
								sizeMode === "compact" ? 4 : sizeMode === "xlarge" ? 10 : 6;
							const displayText =
								option.length > maxTextLength
				...
				<div className="flex items-center gap-2 mt-4">
					...
					>
						...
						/>
						{isSpinning ? `${t.widgets.spin}中...` : t.widgets.start}
					</Button>
					{result && (
						<Button
							size={sizeMode === "compact" ? "sm" : "default"}
