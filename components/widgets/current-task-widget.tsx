...
import { Clock, Flame, Pause, Play, Target, Timer } from "lucide-react";
 ...
 * @returns 当前任务组件
 */
export function CurrentTaskWidget({
	id: _id,
	config: _config,
	className,
}: BaseWidgetProps) {
	...
	const currentTask = useMemo(() => {
		const todayTasks = state.tasks.filter(
			(t) => t.date === today && t.status === "pending" && !t.isAllDay,
		);
	...
	const getTimeUntilStart = useMemo(() => {
		if (!currentTask || currentTask.isActive || !currentTask.startTime)
			return null;
				...
				<>
					...
					<div
						className={cn(
							...
							isNarrow
								? "flex-col items-center justify-center p-2 gap-2"
								: "items-center gap-4",
							sizeMode === "compact" && !isNarrow
								? "p-2"
								: isLarge
									? "p-5"
									: "p-4",
						...
						<div className={cn("flex-1 min-w-0", isNarrow && "text-center")}>
							<div
								className={cn(
									...
									isNarrow && "justify-center flex-wrap",
								)}
							>
								<span
									className={cn(
									...
									style={
										!isPomodoroActive
											? { backgroundColor: tagColor, color: "white" }
											: {}
									}
								>
									{isPomodoroRunning
