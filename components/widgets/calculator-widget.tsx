...
export function CalculatorWidget({
	...
	const addToHistory = useCallback(
		(expression: string, result: string) => {
			setHistory((prev) => {
				const newHistory = [{ expression, result }, ...prev].slice(
					0,
					maxHistoryItems,
				);
				return newHistory;
	...
	const backspace = useCallback(() => {
		setState((prev) => {
			if (
				prev.display.length === 1 ||
				(prev.display.length === 2 && prev.display[0] === "-")
			) {
	...
	const buttonHeight = useMemo(() => {
		...
		const displayHeight =
			sizeMode === "compact" ? 60 : sizeMode === "xlarge" ? 100 : 80;
		const padding = 16;
		const gap = sizeMode === "compact" ? 2 : 4;

		const availableHeight =
			containerSize.height - headerHeight - displayHeight - padding;
		const buttonRows = 5;
	...
	const buttonFontSize = useMemo(() => {
		if (containerSize.width < 160 || containerSize.height < 220)
			return "text-xs";
		switch (sizeMode) {
			case "compact":
				return "text-sm";
	...
	const displayHistory =
		showHistoryPanel && history.length > 0 && sizeMode !== "compact";
				...
				<div className="border-b border-border bg-muted/20 max-h-24 overflow-y-auto">
					...
					<div className="p-2 space-y-1">
						{history.map((item, index) => (
							<div
								key={index}
								className="text-xs flex justify-between px-2 py-1 rounded bg-muted/50"
							>
			...
			>
				{buttons.map((btn, index) => (
					...
					>
						{btn.icon ? (
							<btn.icon
								className={sizeMode === "xlarge" ? "w-6 h-6" : "w-5 h-5"}
							/>
						) : (
							btn.label
						)}
