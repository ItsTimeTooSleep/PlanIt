 ...
 * @returns 搜索输入框组件
 */
export function SearchWidget({ id: _id, config, className }: BaseWidgetProps) {
	...
	const showEngineButtons =
		showQuickButtons && sizeMode !== "compact" && containerSize.height >= 80;
	const displayEngineCount =
		containerSize.width >= 450 ? 6 : containerSize.width >= 350 ? 5 : 4;
			...
			>
				{showSelect && (
					<Select
						...
						onValueChange={(v) => setSelectedEngine(v as SearchEngine)}
					>
						<SelectTrigger
							className={cn("w-auto min-w-[80px]", inputHeight, fontSize)}
						>
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
							...
							<div className="max-h-40 overflow-y-auto">
								{searchHistory.map((item, index) => (
									<button
										key={index}
										className={cn(
											"w-full text-left px-3 py-2 hover:bg-muted/50 transition-colors truncate",
											fontSize,
										)}
				...
				>
					{SEARCH_ENGINES.slice(0, displayEngineCount).map((engine) => (
						<Button
							...
							className={cn(
								"text-xs px-2 h-7",
								selectedEngine === engine.id && "shadow-sm",
							)}
