				...
				if (startTimeRef.current !== null) {
					...
					if (mode === "countdown") {
						const newTime = Math.max(
							0,
							timeAtStartRef.current - elapsedSeconds,
						);
						setTime(newTime);
						if (newTime === 0) {
							setIsRunning(false);
		...
		>
				...
				<div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
					...
					<div className="flex items-center gap-1 bg-muted rounded-lg p-0.5">
						<Button
							...
							className={cn(
								"text-xs",
								mode === "stopwatch" && "bg-background shadow-sm",
							)}
							onClick={() => {
								setMode("stopwatch");
								setTime(0);
						...
						<Button
							...
							className={cn(
								"text-xs",
								mode === "countdown" && "bg-background shadow-sm",
							)}
							onClick={() => {
								setMode("countdown");
								setTime(initialTime);
			...
			<div className="flex-1 flex flex-col items-center justify-center p-4">
				...
				<div className="relative mb-4">
					...
					>
						<span
							className={cn("font-mono font-bold tracking-tight", timeFontSize)}
						>
							{formatTime(time)}
						</span>
					</div>
				...
				<div className="flex items-center gap-2">
					{!isRunning ? (
						<Button size={buttonSize} className="rounded-full" onClick={start}>
							<Play
								className={sizeMode === "compact" ? "w-4 h-4" : "w-5 h-5"}
							/>
						</Button>
					) : (
						<Button
