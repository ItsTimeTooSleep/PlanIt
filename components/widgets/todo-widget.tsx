							...
							<div className="space-y-0.5">
								{showOverdueSection && (
									<div className="mb-2">
										<div
											className={cn(
												"font-medium text-destructive mb-1 px-1",
												taskTimeSize,
											)}
										>
										...
										<div className="space-y-0.5">
											{overdueTasks.map((task) => (
												<div
													...
													className={cn(
														"rounded-lg hover:bg-muted/60 transition-colors cursor-pointer group border border-transparent hover:border-border/50 flex items-start gap-2",
														taskItemPadding,
													)}
												>
													<Checkbox
													...
													<ExternalLink
														className={cn(
															"text-muted-foreground/0 group-hover:text-muted-foreground/50 transition-all flex-shrink-0 mt-0.5",
															sizeMode === "compact"
																? "w-2.5 h-2.5"
																: "w-3 h-3",
														)}
								...
								{todayTasks.length > 0 ? (
									<div>
										{showOverdueSection && overdueTasks.length > 0 && (
											<div
												className={cn(
													"font-medium text-muted-foreground mb-1 px-1 pt-1",
													taskTimeSize,
												)}
											>
									...
									<div
										className={cn(
											"text-center",
											sizeMode === "compact" ? "py-3" : "py-6",
										)}
									>
										<CheckSquare
											className={cn(
												"mx-auto text-muted-foreground/30 mb-2",
												sizeMode === "compact" ? "w-5 h-5" : "w-8 h-8",
											)}
										/>
										<p className={cn("text-muted-foreground", taskTitleSize)}>
											{t.todo.noTasks}
										</p>
									</div>
								) : null}
								...
								{showFutureLink && (
									<div
										...
										className={cn(
											"rounded-lg hover:bg-muted/60 transition-colors cursor-pointer group border border-border/30 hover:border-border/50 flex items-center justify-between",
											taskItemPadding,
											"mt-2",
										)}
									>
										<div
											className={cn(
												"text-muted-foreground group-hover:text-foreground transition-colors flex items-center gap-1.5",
												taskTimeSize,
											)}
										>
										...
										<div className="flex items-center gap-1">
											...
											<ExternalLink
												className={cn(
													"text-muted-foreground/50 group-hover:text-muted-foreground transition-all",
													sizeMode === "compact" ? "w-2.5 h-2.5" : "w-3 h-3",
												)}
											/>
										</div>
									</div>
