...
export function CollapsibleWrapper({
	...
	console.log("[CollapsibleWrapper] render", {
		isExpanded,
		contentHeight,
		isInitialized,
	});
	...
	useEffect(() => {
		...
		if (prevExpandedRef.current !== isExpanded) {
			...
			console.log("[CollapsibleWrapper] calling onCollapsedChange", {
		...
		if (contentRef.current) {
			const resizeObserver = new ResizeObserver((entries) => {
				for (const entry of entries) {
					console.log("[CollapsibleWrapper] ResizeObserver", {
	...
	const totalHeight =
		contentHeight !== undefined && headerHeight > 0
			? headerHeight + contentHeight
			: undefined;

	console.log("[CollapsibleWrapper] height calculation", {
		headerHeight,
		contentHeight,
		collapsedHeight,
		totalHeight,
