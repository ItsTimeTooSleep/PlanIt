...
function findSigFile(dir, pattern) {
	try {
		...
		const sigFile = files.find(
			(f) => f.endsWith(".sig") && f.includes(pattern),
		);
		if (sigFile) {
...
function findMacOsSigsInRoot() {
	...
	for (const dir of searchDirs) {
		try {
			...
			const aarch64SigFile = files.find(
				(f) =>
					f.endsWith(".sig") && (f.includes("aarch64") || f.includes("arm64")),
			);
			...
			if (aarch64SigFile && !result.aarch64) {
				result.aarch64 = fs
					.readFileSync(path.join(dir, aarch64SigFile), "utf8")
					.trim();
...
function main() {
	...
	if (emptyWindows) {
		if (template.platforms["windows-x86_64"]) {
			...
			template.platforms["windows-x86_64"].url =
				`https://github.com/itstimetoosleep/PlanIt/releases/download/v${version}/PlanIt_${version}_x64-setup.exe`;
		}
	...
	if (macosArtifactsDir) {
		...
		macX64Sig =
			findSigFileRecursive(macosArtifactsDir, "x64") ||
			findSigFileRecursive(macosArtifactsDir, "x86_64");
		macArm64Sig =
			findSigFileRecursive(macosArtifactsDir, "aarch64") ||
			findSigFileRecursive(macosArtifactsDir, "arm64");
	} else {
	...
	if (macX64Sig && template.platforms["darwin-x86_64"]) {
		...
		template.platforms["darwin-x86_64"].url =
			`https://github.com/itstimetoosleep/PlanIt/releases/download/v${version}/PlanIt_${version}_x64.app.tar.gz`;
	...
	if (macArm64Sig && template.platforms["darwin-aarch64"]) {
		...
		template.platforms["darwin-aarch64"].url =
			`https://github.com/itstimetoosleep/PlanIt/releases/download/v${version}/PlanIt_${version}_aarch64.app.tar.gz`;
