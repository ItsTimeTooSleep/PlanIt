...
function generateData(days: number) {
	...
	for (let i = days - 1; i >= 0; i--) {
		...
		const previousValue = Math.max(
			1,
			Math.min(8, currentValue - 0.5 + (Math.random() - 0.5)),
		);
...
export function AnalyticsReveal() {
	...
	const visibleDays = Math.max(
		1,
		Math.min(30, Math.ceil(scrollProgress * 29) + 1),
	);
	...
	const avgCurrent =
		visibleData.length > 0
			? (
					visibleData.reduce((sum, d) => sum + d.current, 0) /
					visibleData.length
				).toFixed(1)
			: "0";
	const avgPrevious =
		visibleData.length > 0
			? (
					visibleData.reduce((sum, d) => sum + d.previous, 0) /
					visibleData.length
				).toFixed(1)
			: "0";
	const improvement = (
		((Number(avgCurrent) - Number(avgPrevious)) / Number(avgPrevious)) *
		100
	).toFixed(0);
					...
					<div className="bg-card rounded-2xl border border-border p-6 shadow-lg">
						...
						<div className="flex items-center justify-between mb-6 flex-wrap gap-4">
							<div className="flex items-center gap-6">
								...
								<div>
									...
									<p
										className={`text-2xl font-bold ${Number(improvement) > 0 ? "text-emerald-500" : "text-red-500"}`}
									>
						...
						<div className="w-full overflow-hidden">
							...
							>
								...
								{yTicks.map((tick) => {
									const y =
										padding.top + graphHeight - (tick / maxValue) * graphHeight;
									return (
										<g key={tick}>
											<line
												x1={padding.left}
								...
								{visibleData.map((d, i) => {
									...
									const showLabel =
										...
										i % Math.ceil(visibleData.length / 7) === 0 ||
										i === visibleData.length - 1;
									if (!showLabel) return null;

									const x =
										padding.left +
										(i / Math.max(visibleData.length - 1, 1)) * graphWidth;
									return (
										<text
											key={i}
											x={x}
								...
								{visibleData.map((d, i) => {
									...
									const y =
										padding.top +
										graphHeight -
										(d.current / maxValue) * graphHeight;
						...
						<div className="flex items-center justify-center gap-6 mt-4 text-[11px]">
							...
							<div className="flex items-center gap-2">
								<div
									className="w-4 h-0.5 bg-muted-foreground/30 rounded"
									style={{
										backgroundImage:
											"repeating-linear-gradient(90deg, currentColor 0, currentColor 4px, transparent 4px, transparent 6px)",
									}}
								/>
