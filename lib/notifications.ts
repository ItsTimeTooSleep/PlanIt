...
const firedNotifications: Set<string> = new Set();
...
async function checkAndRequestPermission(): Promise<boolean> {
	if (isDesktop()) {
		try {
			...
			console.log(
				`${LOG_PREFIX} Tauri notification permission status:`,
				granted,
			);

			if (!granted) {
				console.log(
					`${LOG_PREFIX} Requesting Tauri notification permission...`,
				);
				const permission = await requestPermission();
				granted = permission === "granted";
				console.log(
					`${LOG_PREFIX} Tauri permission request result:`,
					permission,
				);
			}
			...
			console.error(
				`${LOG_PREFIX} Error checking/requesting Tauri permission:`,
				error,
			);
			return false;
		}
		...
		if (!("Notification" in window)) {
			console.warn(
				`${LOG_PREFIX} Notification API not supported in this browser`,
			);
			return false;
		}
...
async function showNotification(
	...
	console.log(`${LOG_PREFIX} Attempting to show notification:`, {
		title,
		body,
		tag,
	});

	if (firedNotifications.has(tag)) {
		console.log(
			`${LOG_PREFIX} Notification with tag ${tag} already fired, skipping to prevent duplicates`,
		);
		return;
	}
	...
	if (isDesktop()) {
		try {
			...
			try {
				...
				console.error(
					`${LOG_PREFIX} Fallback notification also failed:`,
					fallbackError,
				);
			}
		}
		...
		if (permission !== "granted") {
			console.warn(
				`${LOG_PREFIX} Notification permission not granted, cannot show notification`,
			);
			return;
		}
		...
		try {
			...
			console.error(
				`${LOG_PREFIX} Failed to create browser notification:`,
				error,
			);
		}
	}
...
export function scheduleTaskNotification(
	...
	if (typeof window === "undefined") {
		console.warn(
			`${LOG_PREFIX} Window is undefined (SSR), skipping notification scheduling`,
		);
		return;
	}
	...
	console.log(
		...
		isDesktopEnv ? "Desktop (Tauri)" : "Browser",
	);

	if (!isDesktopEnv) {
		if (!("Notification" in window)) {
			console.warn(
				`${LOG_PREFIX} Notification API not supported, cannot schedule notification`,
			);
			return;
		}
		...
		if (permission !== "granted") {
			console.warn(
				`${LOG_PREFIX} Permission not granted (${permission}), cannot schedule notification`,
			);
			return;
		}
	}
	...
	if (!task.startTime || !task.date) {
		console.warn(
			`${LOG_PREFIX} Task missing startTime or date, skipping notification`,
		);
		return;
	}
	...
	console.log(
		`${LOG_PREFIX} Cancelled any existing notification for task ${task.id}`,
	);
	...
	if (
		safeSettings.advanceMinutes !== null &&
		safeSettings.advanceMinutes !== undefined
	) {
		const advanceDate = new Date(
			taskStartDate.getTime() - safeSettings.advanceMinutes * 60 * 1000,
		);
		const advanceDelay = advanceDate.getTime() - now;

		console.log(
			`${LOG_PREFIX} Advance notification time:`,
			advanceDate.toISOString(),
		);
		...
		if (advanceDelay >= 0) {
			scheduled.advanceTimeoutId = setTimeout(() => {
				...
				showNotification(
					...
					console.error(
						`${LOG_PREFIX} Error in advance showNotification promise:`,
						error,
					);
				});
		...
		if (startDelay >= 0) {
			scheduled.startTimeoutId = setTimeout(() => {
				...
				showNotification(
					...
					console.error(
						`${LOG_PREFIX} Error in start showNotification promise:`,
						error,
					);
				});
				const current = scheduledIds.get(task.id);
		...
		if (endDelay >= 0) {
			scheduled.endTimeoutId = setTimeout(() => {
				...
				showNotification(
					...
					console.error(
						`${LOG_PREFIX} Error in end showNotification promise:`,
						error,
					);
				});
				const current = scheduledIds.get(task.id);
	...
	console.log(
		`${LOG_PREFIX} Total tasks with scheduled notifications:`,
		scheduledIds.size,
	);
}
...
export function cancelTaskNotification(taskId: string) {
	...
	if (existing) {
		...
		firedNotifications.delete(`${taskId}-advance`);
		firedNotifications.delete(`${taskId}-start`);
		firedNotifications.delete(`${taskId}-end`);
	}
}
 ...
 */
export function cleanupExpiredNotifications(_currentTime: number = Date.now()) {
	const idsToRemove: string[] = [];

	scheduledIds.forEach((scheduled, taskId) => {
		let allTimersFired = true;

		if (scheduled.advanceTimeoutId) allTimersFired = false;
		if (scheduled.startTimeoutId) allTimersFired = false;
		if (scheduled.endTimeoutId) allTimersFired = false;
	...
	console.log(
		`${LOG_PREFIX} Cleaned up ${idsToRemove.length} expired notification entries`,
	);
}
