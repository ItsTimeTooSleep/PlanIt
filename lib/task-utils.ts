...
export function expandRepeatTasks(task: Task, maxDays = 90): Task[] {
	...
	const endDate = task.repeatRule.endDate
		? parseISO(task.repeatRule.endDate)
		: addDays(base, maxDays);
	...
	if (task.date && task.dueDate) {
		...
		dueDateOffsetDays = Math.floor(
			(dueDate.getTime() - planDate.getTime()) / (1000 * 60 * 60 * 24),
		);
	}
	...
	while (iteration < 500) {
		...
		// 对于周重复，只生成匹配的星期几
		if (
			task.repeatRule.frequency === "weekly" &&
			task.repeatRule.weekdays?.length
		) {
...
export function formatDuration(minutes: number, lang: "zh" | "en"): string {
	...
	const m = minutes % 60;
	if (m === 0)
		return lang === "zh" ? `${h} 小时` : `${h} hr${h > 1 ? "s" : ""}`;
 ...
 */
export function getRecurringTaskIds(
	currentTask: Task,
	allTasks: Task[],
): string[] {
 ...
 */
export function isPartOfRecurringGroup(
	currentTask: Task,
	allTasks: Task[],
): boolean {
	// 方案1：当前任务本身有重复规则
	...
	const baseCreatedAt = currentTask.createdAt.slice(0, 19);
	const hasMatchingTasks = allTasks.some(
		(t) =>
			t.id !== currentTask.id &&
