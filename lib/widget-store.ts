...
function wouldCollide(
	...
	for (const other of otherWidgets) {
		...
		const otherHeight =
			other.collapsed && other.actualHeight
				? other.actualHeight
...
export function useWidgetStoreState(): WidgetStoreContextValue {
	...
	const pushToHistory = useCallback(
		(layouts: WidgetLayout[], activeLayoutId: string | null) => {
			console.log(
				"[pushToHistory] called, isUndoRedo:",
				isUndoRedoRef.current,
				"isBatch:",
				isBatchUpdateRef.current,
			);
			...
			console.log(
				"[pushToHistory] saved, history length:",
				newHistory.length,
				"index:",
	...
	const endBatchUpdate = useCallback(() => {
		...
		if (batchStartStateRef.current) {
			...
			if (startKey !== currentKey) {
				...
				console.log(
					"[endBatchUpdate] saved, history length:",
					newHistory.length,
					"index:",
					historyIndexRef.current,
				);
	...
	const getWidget = useCallback(
		(id: string): WidgetInstance | undefined => {
	...
	const updateWidget = useCallback(
		(id: string, updates: Partial<WidgetInstance>): void => {
			const now = new Date().toISOString();
			setStateWithHistory((prev) => ({
				...
				layouts: prev.layouts.map((layout) => {
					if (
						...
						(!prev.activeLayoutId && layout === prev.layouts[0])
					) {
						return {
							...layout,
	...
	const moveWidget = useCallback(
		(id: string, position: PercentPosition): void => {
	...
	const resizeWidget = useCallback(
		(id: string, size: PercentSize): void => {
	...
	const updateWidgetZIndex = useCallback(
		(id: string, zIndex: number): void => {
	 ...
	 */
	const updateWidgetCollapsedState = useCallback(
		(id: string, collapsed: boolean, actualHeight?: number): void => {
			const now = new Date().toISOString();
			setStateWithHistory((prev) => {
				const layoutId = prev.activeLayoutId || prev.layouts[0]?.id;
				const layout = prev.layouts.find((l) => l.id === layoutId);
				...
				const originalHeight = percentToPixel(
				...
				if (collapsed) {
					...
					const prevCollapsedHeight =
						targetWidget.actualHeight || collapsedHeight;
					heightDiff = originalHeight - prevCollapsedHeight;
				}

				const targetY = percentToPixel(
				...
				const updatedWidgets = layout.widgets.map((widget) => {
					if (widget.id === id) {
						return {
							...widget,
							collapsed,
							actualHeight: collapsed ? collapsedHeight : undefined,
							updatedAt: now,
						};
					}
	 ...
	 * @param locked - 是否锁定
	 */
	const toggleWidgetLockState = useCallback(
						...
						if (layout.id === layoutId) {
							const newWidgets = widgetIds
								.map((id, index) => {
									...
									return widget
										? { ...widget, zIndex: index + 1, updatedAt: now }
										: widget;
								})
								.filter(Boolean) as WidgetInstance[];

							const remainingWidgets = layout.widgets.filter(
	...
	const createLayout = useCallback(
		(name: string, canvasSize?: CanvasSize): WidgetLayout => {
			const now = new Date().toISOString();
	...
	const updateCanvasSize = useCallback(
		(id: string, canvasSize: CanvasSize): void => {
			const now = new Date().toISOString();
			setStateWithHistory((prev) => ({
				...prev,
				layouts: prev.layouts.map((layout) =>
					layout.id === id
						? { ...layout, canvas: canvasSize, updatedAt: now }
						: layout,
				),
			}));
		},
