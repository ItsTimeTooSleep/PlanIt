 ...
 * @returns 更新插件或 null
 */
async function getTauriUpdater(): Promise<{
 ...
 * @returns 进程插件或 null
 */
async function getTauriProcess(): Promise<{
...
export class UpdaterManager {
	...
	public static getInstance(messages?: UpdateMessages): UpdaterManager {
		if (!UpdaterManager.instance) {
			...
			UpdaterManager.instance.messages = {
				...UpdaterManager.instance.messages,
				...messages,
			};
		}
	...
	public updateLastCheckDate(): void {
		try {
			...
			console.error(
				"[UpdaterManager] Failed to update last check date:",
				error,
			);
		}
	}
	...
	public async checkForUpdates(
		showToastIfLatest: boolean = true,
		forceCheck: boolean = false,
	): Promise<boolean> {
		if (!isDesktop()) {
			console.warn(
				"[UpdaterManager] Update functionality not available on web platform",
			);
			return false;
		}
		...
		try {
			...
			if (update) {
				...
				if (!forceCheck && this.isVersionSkipped(updateData.version)) {
					console.log(
						`[UpdaterManager] Version ${updateData.version} has been skipped`,
					);
					return false;
				}
