...
export function findNonOverlappingPosition(
	...
	const checkOverlap = (pos: Position): boolean => {
		...
		return existingWidgets.some(
			(w) => w.id !== newWidget.id && doWidgetsOverlap(testWidget, w),
		);
	};
	...
	while (checkOverlap(position) && attempts < maxAttempts) {
		...
		if (
			position.y + newWidget.size.height >
			canvasSize.height - CANVAS_PADDING
		) {
	...
	sorted.forEach((widget, index) => {
		...
		const height =
			isCollapsed && collapsedHeight
				? collapsedHeight
				: widget.actualHeight
					? percentToPixel(widget.actualHeight, canvasSize.height)
		...
		const newWidget: WidgetInstance = {
			...
			position: {
				x: pixelToPercent(
					CANVAS_PADDING + (index % 2) * (columnWidth + gap),
					canvasSize.width,
				),
