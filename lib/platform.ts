...
const webAPI: PlatformAPI = {
	...
	registerGlobalShortcut: async () => {
		console.warn(
			"[Platform] Global shortcuts are not supported on web platform",
		);
		return false;
	},
	...
	setWindowState: async () => {
		console.warn(
			"[Platform] Window controls are not supported on web platform",
		);
	},
...
export async function getPlatformAPI(): Promise<PlatformAPI> {
	...
	try {
		...
		console.error(
			"[Platform] Failed to load Tauri API, falling back to web API:",
			error,
		);
		cachedPlatformAPI = webAPI;
		return webAPI;
