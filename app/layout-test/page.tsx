...
import {
	type ContentVisibilityOption,
	DEFAULT_TASK_CONTENT_CONFIGS,
	LayoutTestWidget,
	type LayoutTestWidgetRef,
} from "@/components/widgets/layout-test-widget";
import { cn } from "@/lib/utils";

export default function LayoutTestPage() {
	const resizableRef = useRef<LayoutTestWidgetRef>(null);
	const [layoutInfo, setLayoutInfo] =
		useState<ReturnType<LayoutTestWidgetRef["getLayoutInfo"]>>();
	...
	const toggleRequired = (id: string) => {
		setContentOptions((prev) =>
			prev.map((opt) =>
				opt.id === id ? { ...opt, required: !opt.required } : opt,
			),
		);
	};
							...
							{layoutInfo && (
								<>
									<div className="space-y-1">
										...
										<p>
											容器: {Math.round(layoutInfo.containerWidth)} ×{" "}
											{Math.round(layoutInfo.containerHeight)}
										</p>
										<p>内容区: {Math.round(layoutInfo.contentAreaWidth)}</p>
										<p className="text-orange-600 font-medium">
											最小尺寸: {Math.round(minSize.minWidth)} ×{" "}
											{Math.round(minSize.minHeight)}
										</p>
									</div>
									...
									<div className="border-t pt-3 space-y-1">
										...
										<p>
											标题截断: {layoutInfo.needsTitleTruncate ? "是" : "否"}
										</p>
